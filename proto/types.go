...
type Server struct {
	...
	ClientAgg	map[uint]*mpc.Aggregator //[yoza] Type uint right now. Change it to Uuid for a secret key as the ID.
	KeyToID		map[Uuid]uint
	storedClientAgg map[uint]*mpc.Aggregator
...
type PublishArgs struct {
	...
	Epoch  uint
	Agg    *mpc.Aggregator
	ClientAgg map[uint]*mpc.Aggregator
}
