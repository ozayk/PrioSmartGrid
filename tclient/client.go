...
func connectToServer(cfg *config.Config, serverIdx int) *rpc.Client {
	...
	log.Print("Connected! to address:%v", serverAddr)
	return client
}
...
func main() {
	...
	ClientID := flag.Int("ClientID", -1, "Unique ID of the client")

	flag.Parse()

	utils.C_ID = int64(*ClientID)
	if utils.C_ID == -1 {
		log.Printf("C_ID set to default: %v", utils.C_ID)
	} else {
		log.Printf("C_ID populated with: %v", utils.C_ID)
	}
