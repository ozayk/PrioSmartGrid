...
func runAt(hosts []string, do commandFunc) {
	...
	log.Printf("[yoza] runAt. number og hosts = %d", n)
	for i := 0; i < n; i++ {
		<-c
	}
}
...
func runAtServers(cfg *config.Config, do commandFunc) {
	...
	for i := 0; i < n; i++ {
		...
		log.Printf("[yoza] runAtServers. Host = %s", hosts[i])

	}

	log.Printf("[yoza] runAtServers exiting...")
...
func runAtClients(cfg *config.Config, do commandFunc) {
	...
	for i := 0; i < n; i++ {
		...
		log.Printf("[yoza] runAtClients. Host = %s", hosts[i])

	}
	log.Printf("[yoza] runAtClients exiting...")
...
func runStartClients(cfg *config.Config) {
	runAtClients(cfg, func(i int) string {
		return fmt.Sprintf("~/tclient -config ~/config.conf -log /tmp/client-out-%v.log -outfile /tmp/request-%v.dat", i, i)
	})

	runAtClients(cfg, func(i int) string {
		//return fmt.Sprintf("~/tclient -config ~/config.conf -log /tmp/client-%v.log -infile /tmp/request.dat -n 600 -forever", i)
		return fmt.Sprintf("~/tclient -config ~/config.conf -log /tmp/client-in-%v.log -infile /tmp/request-%v.dat -n 100 -ClientID %v", i, i, i)
	})
}
