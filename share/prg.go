...
func (g *GenPRG) Share(mod *big.Int, value *big.Int) []*big.Int {
	...
	delta := new(big.Int)
	log.Printf("[yoza] delta value before loop is: ", delta)
	log.Printf("[yoza] mod for PRG is: ", mod)
	log.Printf("[yoza] Original VALUE for PRG is: ", value)
	for i := 0; i < g.nServers; i++ {
		out[i] = g.rand[i].RandInt(mod)
		delta.Add(delta, out[i])
		log.Printf("[yoza] out value in loop is: ", out[i])
	}
	...
	log.Printf("[yoza] out value of leader after loop is: ", g.leaderIdx, out[g.leaderIdx])
	log.Printf("[yoza] out value is: ", out)
